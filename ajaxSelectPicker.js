...
!(function ($, window) {
    $.ajaxSelectPicker = function (element, options) {
        ...
        plugin.init = function () {
            // Instantiate the cached results object.
            plugin.cachedData = {};

            // Instantiate the history of the select element.
            plugin.states = [];

            // Instantiate the query states.
            plugin.query = '';
            plugin.previousQuery = '';

            // Instantiate the loading element.
            plugin.$loading = $();
            ...
            $.map(depreciatedMap, function(map) {
              ...
              else if (plugin.ajaxOptions[map.from]) {
            ...
            } else if (plugin.ajaxOptions.ajaxOptions.url == null) {
                    ...
                    timeout = setTimeout(function () {
                        ...
                        ajaxParams.success = function (data) {
                          ...
                          if (processedData) {
                            ...
                            plugin.log(['ajaxSelectPicker: Unable to process data.', data], true);
                          }
                          plugin.restoreState();
                        };
         ...
         * @return {void}
         */
        plugin.replaceValue = function (obj, needle, value, options) {
          options = $.extend({
            recursive: true,
            depth: false,
            limit: false
         ...
         * @return {void}
         */
        plugin.saveState = function (keepPreviousStates) {
          var selected = [];
         ...
         */
        plugin.processData = function (data, query) {
          ...
          if ($.isPlainObject(data)) {
            clone = selected.concat($.map(data, function(value) { return [value] }));
          }
          else if ($.isArray(data)) {
            clone = selected.concat(data);
          }
          else {
            plugin.log(['ajaxSelectPicker: The data type passed was not an Array or Object.', data], true);
            return false;
          }
          ...
          if ($.isFunction(plugin.ajaxOptions.preprocessData)) {
            ...
            if (!$.isArray(preprocessedData) || !preprocessedData.length) {
              plugin.log(['ajaxSelectPicker: The preprocessData callback did not return an array or was empty.', data, preprocessedData], true);
              return false;
            }
          }
            ...
            if ($.isPlainObject(item)) {
              ...
              // Ensure item has a "value" and is unique.
                ...
                if (seenValues.indexOf(item.value) === -1) {
                  ...
                  this.log(['ajaxSelectPicker: Duplicate item found, ignoring.', item]);
                }
              }
              else {
                this.log(['ajaxSelectPicker: Data item must have a "value" property, skipping.', item], true);
              }
            }
          }
          ...
          if ($.isFunction(plugin.ajaxOptions.processData)) {
            ...
            if (!$.isArray(processedData) || !processedData.length) {
              plugin.log(['ajaxSelectPicker: The processedData callback did not return an array or was empty.', data, filteredData, processedData], true);
              return false;
            }
          }
         ...
         */
        plugin.buildOptions = function (data) {
          var a, i, selected, l = data.length, output = '';
          var $wrapper = $('<select/>');
